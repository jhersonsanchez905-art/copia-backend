...
async def listar_proveedores(db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return await ProveedorService(db).listar()

@router.get("/proveedores/{id_proveedor}", response_model=ProveedorOut)
async def obtener_proveedor(id_proveedor: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
async def crear_proveedor(data: ProveedorCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await ProveedorService(db).crear(data)

@router.put("/proveedores/{id_proveedor}", response_model=ProveedorOut)
async def actualizar_proveedor(id_proveedor: int, data: ProveedorUpdate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
...
async def eliminar_proveedor(id_proveedor: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
...
async def listar_insumos_proveedor(db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
async def obtener_insumo_proveedor(id_insumo_proveedor: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
async def crear_insumo_proveedor(data: InsumoProveedorCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await InsumoProveedorService(db).crear(data)

@router.delete("/insumos-proveedor/{id_insumo_proveedor}", status_code=204)
async def eliminar_insumo_proveedor(id_insumo_proveedor: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
