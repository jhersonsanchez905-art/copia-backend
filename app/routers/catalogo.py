...
async def listar_roles(db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return await RolService(db).listar()

@router.get("/roles/{id_rol}", response_model=RolOut)
async def obtener_rol(id_rol: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
async def crear_rol(data: RolCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await RolService(db).crear(data)

@router.delete("/roles/{id_rol}", status_code=204)
async def eliminar_rol(id_rol: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
...
async def listar_clientes(db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return await ClienteService(db).listar()

@router.get("/clientes/{id_cliente}", response_model=ClienteOut)
async def obtener_cliente(id_cliente: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
async def crear_cliente(data: ClienteCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await ClienteService(db).crear(data)

@router.put("/clientes/{id_cliente}", response_model=ClienteOut)
async def actualizar_cliente(id_cliente: int, data: ClienteUpdate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
...
async def eliminar_cliente(id_cliente: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    if not await ClienteService(db).eliminar(id_cliente):
        raise HTTPException(status_code=404, detail="Cliente no encontrado")

# --- Marcas ---
@router.get("/marcas", response_model=List[MarcaOut])
async def listar_marcas(db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return await MarcaService(db).listar()

@router.post("/marcas", response_model=MarcaOut, status_code=201)
async def crear_marca(data: MarcaCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await MarcaService(db).crear(data)

@router.delete("/marcas/{id_marca}", status_code=204)
async def eliminar_marca(id_marca: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    if not await MarcaService(db).eliminar(id_marca):
        raise HTTPException(status_code=404, detail="Marca no encontrada")

# --- Unidades de Medida ---
@router.get("/unidades-medida", response_model=List[UnidadMedidaOut])
async def listar_unidades(db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return await UnidadMedidaService(db).listar()

@router.post("/unidades-medida", response_model=UnidadMedidaOut, status_code=201)
async def crear_unidad(data: UnidadMedidaCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await UnidadMedidaService(db).crear(data)

@router.delete("/unidades-medida/{id_unidad}", status_code=204)
async def eliminar_unidad(id_unidad: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
...
async def listar_clasificaciones(db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return await ClasificacionService(db).listar()

@router.post("/clasificaciones", response_model=ClasificacionOut, status_code=201)
async def crear_clasificacion(data: ClasificacionCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await ClasificacionService(db).crear(data)

@router.delete("/clasificaciones/{id_clasificacion}", status_code=204)
async def eliminar_clasificacion(id_clasificacion: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
...
async def crear_categoria(data: CategoriaCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await CategoriaService(db).crear(data)

@router.delete("/categorias/{id_categoria}", status_code=204)
async def eliminar_categoria(id_categoria: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
...
async def listar_metodos_pago(db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    return await MetodoPagoService(db).listar()

@router.post("/metodos-pago", response_model=MetodoPagoOut, status_code=201)
async def crear_metodo_pago(data: MetodoPagoCreate, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
    return await MetodoPagoService(db).crear(data)

@router.delete("/metodos-pago/{id_metodo_pago}", status_code=204)
async def eliminar_metodo_pago(id_metodo_pago: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(require_rol("administrador"))):
